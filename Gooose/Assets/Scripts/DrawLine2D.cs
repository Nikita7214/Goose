...
{
	[SerializeField] private bool _IsAlreadyDraw = false;
	[SerializeField]
	...
	void OnMouseDrag()
	{
		...
		m_LineRenderer.GetComponent<LineRenderer>().SetPosition(0,
			new Vector3(transform.position.x + (GetComponent<SpriteRenderer>().bounds.size.x) / 2,
			transform.position.y,
			transform.position.z));
		m_LineRenderer.GetComponent<LineRenderer>().SetPosition(1, Camera.main.ScreenToWorldPoint(Input.mousePosition) + Vector3.forward * 10);

		tempEdges = m_LineRenderer.GetComponent<EdgeCollider2D>().points;
		...
		m_LineRenderer.GetComponent<EdgeCollider2D>().points = tempEdges;
	}
}
